...
def save_kv_csv(keys, values, job_id, file_name_a, BUCKET_NAME):
    key = f"kv/{job_id}/{file_name_a}.csv"
    df = pd.DataFrame()
    df["Keys"] = keys
    df["Values"] = values
...
def process_response(BUCKET_NAME, job_id, file_name_a, get_table=False, get_kv=True, get_text=False):
    ...
    if get_kv:
        ...
        save_kv_csv(keys, values, job_id,file_name_a, BUCKET_NAME)
    if get_table:
